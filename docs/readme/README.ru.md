...
Первым запросом мастер предлагает выбрать язык диалога и только затем открывает главное меню со всеми этапами 1–8 и пунктом удаления/сброса. Выберите нужный пункт, выполните его, и после завершения мастер вернётся обратно в главное меню. В первом этапе пустой ввод устанавливает все рекомендуемые наборы программ. Во время изменения SSH-порта не закрывайте текущую сессию — сначала проверьте новый вход во втором терминале.
...
| --- | --- | --- |
| `ochenstarik-server-install.sh` | Сначала выбирает язык диалога, затем показывает главное меню установки и удаления | Да; рекомендуемый способ полной настройки |
...
| `ochenstarik-server-uninstall.sh` | Удаляет настройки, службы и данные проекта для повторной установки с начала | Доступен из мастера; запускать только из сохранённой SSH-сессии |
